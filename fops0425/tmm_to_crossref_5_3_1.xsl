    ...
    <xsl:attribute name="book_type">monograph</xsl:attribute>
      <book_metadata language="en">
        ...
        <doi_data>
            ...
            -->
            <xsl:if test="exists(subisbn)">
                <xsl:element name="resource"><xsl:value-of select="concat('https://press.umich.edu/isbn/',subisbn)"/></xsl:element>
            </xsl:if>
        </doi_data>
      </book_metadata>
    </xsl:element>
...
<xsl:template name="contributors">
  <xsl:if test="node()[starts-with(name(), 'authortype')][text()='Author' or contains(text(), 'Editor') or text()='Editor']">
    ...
    <xsl:template match="*[starts-with(name(), 'authorfirstname')]">
        <given_name xmlns="http://www.crossref.org/schema/5.3.1"><xsl:value-of select="substring(.,1,45)"/></given_name>
    </xsl:template>

    <xsl:template match="*[starts-with(name(), 'authorlastname')]">
...
<xsl:template match="printISBN">
    <xsl:element name="isbn" xmlns="http://www.crossref.org/schema/5.3.1">
        <xsl:attribute name="media_type" select="'print'"/>
        <xsl:value-of select="../subisbn"/>
    </xsl:element>
</xsl:template>

<xsl:template match="eISBN">
    <xsl:element name="isbn" xmlns="http://www.crossref.org/schema/5.3.1">
        <xsl:attribute name="media_type" select="'electronic'"/>
        <xsl:value-of select="."/>
    </xsl:element>
</xsl:template>
