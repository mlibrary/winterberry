  ...
  class EmbedLinkFilter < UMPTG::Fulcrum::Filter::ManifestFilter
    ...
    def create_img_actions(reference_node)
      ...
      unless content.empty?
        ...
        table_node = reference_node.xpath("./ancestor::*[local-name()='table'][1]").first
        cell_cnt = table_node.nil? ? 0 : \
            table_node.xpath("count(./descendant::*[local-name()='tr'][1]/*[local-name()='td'])")

        if cell_cnt < 2
          # Convert table to a figure
          if cell_cnt == 0
            p_node  = reference_node.xpath("./ancestor::*[local-name()='p'][1]").first
            replace_node = p_node.nil? ? reference_node : p_node
          else
            section_node  = table_node.xpath("./ancestor::*[local-name()='section'][1]").first
            replace_node = section_node.nil? ? table_node : section_node
          end

          figure_node = Nokogiri::XML::DocumentFragment.parse("<figure></figure>")
          figure_node = figure_node.xpath("./*[local-name()='figure']").first
          figure_node.add_child(reference_node)

          link_container = UMPTG::XML::Pipeline::Actions::EmbedAction.default_container(reference_node, container = 'p')
          link_container.add_child(link_markup)
          figure_node.add_child("<figcaption>" + link_container.to_xml + "</figcaption>")

          action_list << UMPTG::XML::Pipeline::Actions::MarkupAction.new(
                   name: name,
                   reference_node: replace_node,
                   action: :replace_node,
                   markup: figure_node.to_xml,
                   info_message: "#{reference_name}: replace #{replace_node.name} with figure"
               )
        elsif cell_cnt > 1
          # Add link to cell caption
          cell_ndx = reference_node.xpath("count(./ancestor::*[local-name()='td'][1]/preceding-sibling::*)")
          cell_node = reference_node.xpath("./ancestor::*[local-name()='tbody'][1]/*[local-name()='tr' and position()=last()]/*[local-name()='td' and position()=#{cell_ndx+1}]").first

          unless cell_node.nil?
            cell_class = cell_node.first_element_child["class"]
            markup = "<p class=\"#{cell_class}\">" + link_markup + "</p>"

            action_list << UMPTG::XML::Pipeline::Actions::MarkupAction.new(
                     name: name,
                     reference_node: cell_node,
                     action: :add_child,
                     markup: markup,
            ...
            action_list << UMPTG::XML::Pipeline::Actions::SetAttributeValueAction.new(
                     ...
                     info_message: "#{reference_name}: don't embed resource"
                 )
          end
        end
      end
