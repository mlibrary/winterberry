module UMPTG::XML::Reviewer::Filter
  ...
  class PackageFilter < UMPTG::XML::Processor::Filter::Filter
    ...
    def run(xml_doc, args = {})
      ...
      @child_elements.each do |elem|
        ...
        if alist.empty?
          new_act = UMPTG::XML::Processor::Action::Action.new(reference_node: metadata_node)
          new_act.add_warning_msg("#{metadata_node.name} #1: element #{elem} not found.")
        else
          new_act = UMPTG::XML::Processor::Action::Action.new(reference_node: alist.first)
      ...
      if node_list.empty?
        actions << UMPTG::XML::Processor::Action::Action.new(
               name: @name,
        ...
        node_list.each_with_index do |node,ndx|
          actions << UMPTG::XML::Processor::Action::StripAttributeValueAction.new(
                 name: @name,
                 reference_node: node,
                 attribute_name: "href",
      ...
      if node_list.empty?
        ...
        unless node_list.empty?
          ...
          unless node_list.empty?
            ...
            actions << UMPTG::XML::Processor::Action::SetAttributeValueAction.new(
                    name: @name,
    ...
    def remove_duplicates(name, context_node, xpath, msgs, markup = "")
      ...
      if node_list.empty?
        ...
        actions << UMPTG::XML::Processor::Action::NormalizeInsertMarkupAction.new(
               name: name,
        ...
        node_list.each_with_index do |node,ndx|
          ...
          when 0
            ...
            actions << UMPTG::XML::Processor::Action::RemoveElementAction.new(
                      name: name,
                      reference_node: node,
                      action_node: node,
