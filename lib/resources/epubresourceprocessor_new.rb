module UMPTG::Resources
  ...
  class EpubResourceProcessor_NEW
    def self.process(args = {})
      ...
      epub = UMPTG::EPUB_NEW::Archive.new(:epub_file => epub_file)
      ...
      if html_path_update_list.count > 0
        ...
        if opf_content.nil?
          ...
          if manifest_node == nil
            ...
            dest_css_file = File.join(File.dirname(opf_item.name), File.basename(fulcrum_css_file))
...
=end
          ...
          # Save the OPF file.
          epub.add(entry_name: opf_item.name, entry_content: UMPTG::XMLUtil.doc_to_xml(doc))
=begin
...
=end
      ...
      # Save the processed EPUB to a new location.
      epub.save(epub_file: processed_epub_file)
=begin
