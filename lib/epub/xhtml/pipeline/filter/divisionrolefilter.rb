module UMPTG::EPUB::XHTML::Pipeline::Filter
  ...
  class DivisionRoleFilter < UMPTG::XML::Pipeline::Filter
    ...
    XPATH = <<-SXPATH
    //*[
    (local-name()='article' or local-name()='section')
  ...
  puts "#{issue.content.name},role=#{role}"
      if issue.content.name == "article"
        ...
        if role.empty? or role.downcase == "main"
          ...
          when "copyright-page", "division", "note", "titlepage"
          else
            new_role = 'doc-' + epub_type
          end
  puts "#{issue.content.name},type=#{epub_type},new_role=#{new_role}"
