...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    -->
    <xsl:variable name="PROPERTIES"
                  select="'ID,Title,Creator(s),ISBN(s),Publisher,Pub Location,Pub Year,Series,Keywords,Representative Kind,Identifier(s),Copyright Holder,Copyright Status,Holding Contact,Additional Creator(s)'"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//DLPSTEXTCLASS/HEADER">

            <xsl:variable name="isbns" select="FILEDESC/PUBLICATIONSTMT/IDNO[starts-with(@TYPE,'isbn-')]"/>
            ...
            <xsl:variable name="id">
                <xsl:choose>
                    <xsl:when test="exists($isbns[@TYPE='isbn-electronic'])">
                        <xsl:value-of select="translate($isbns[@TYPE='isbn-electronic'],'-','')"/>
                    </xsl:when>
                    <xsl:when test="exists($isbns[@TYPE='isbn-print'])">
                        <xsl:value-of select="translate($isbns[@TYPE='isbn-print'],'-','')"/>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:value-of select="concat($QUOTE,$id,$QUOTE)"/>
            <xsl:call-template name="insertFieldSeparator"/>

            <!-- Title (issuetitle) -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/TITLESTMT/TITLE[not(exists(@TYPE))]"/>
            </xsl:call-template>
            <!--
            ...
            <!-- Creator(s) (author) -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/TITLESTMT/*[local-name()='EDITOR' or local-name()='AUTHOR']"/>
            </xsl:call-template>

            <!-- ISBN(s) -->
            <xsl:variable name="isbn_list">
                <xsl:for-each select="$isbns">
                    <xsl:variable name="v" select="normalize-space(.)"/>
                    <xsl:if test="$v!=''">
                        <xsl:choose>
                            <xsl:when test="@TYPE='isbn-print'">
                                <xsl:value-of select="concat(translate($v,'-',''),' (hardcover)')"/>
                            </xsl:when>
                            <xsl:when test="lower-case(@TYPE)='isbn-electronic'">
                                <xsl:value-of select="concat(translate($v,'-',''),' (ebook)')"/>
                            </xsl:when>
                            <xsl:otherwise>
            ...
            <xsl:value-of select="concat($QUOTE,substring($isbn_list,1,string-length($isbn_list)-1),$QUOTE)"/>
            <xsl:call-template name="insertFieldSeparator"/>

            <!-- Publisher -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/PUBLICATIONSTMT/PUBLISHER"/>
            </xsl:call-template>

            <!-- Pub Location -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/PUBLICATIONSTMT/PUBPLACE"/>
            </xsl:call-template>

            <!-- Pub Year -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/PUBLICATIONSTMT/DATE[not(exists(@TYPE))]"/>
            </xsl:call-template>

            <!-- Series -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="FILEDESC/SERIESSTMT/TITLE"/>
            </xsl:call-template>

            <!-- Keywords -->
            <xsl:call-template name="insertValue">
                <xsl:with-param name="list" select="PROFILEDESC/TEXTCLASS/KEYWORDS"/>
            </xsl:call-template>
