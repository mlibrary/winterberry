...
>
    ...
    <xsl:template match="book">
        ...
        <xsl:variable name="doi" select="normalize-space(./doi)"/>
        <xsl:if test="$doi != ''">
            <xsl:element name="book" namespace="{$NAMESPACE_URL}">
                ...
                <xsl:element name="book_metadata" namespace="{$NAMESPACE_URL}">
                    ...
                    <xsl:for-each select="$isbn_list">
                        <xsl:variable name="pos" select="position()"/>
                        <xsl:variable name="isbn" select="normalize-space($isbn_list[$pos])"/>
                        <xsl:if test="$isbn !=''">
                            <xsl:variable name="fmt" select="normalize-space(translate($format_list[$pos], 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))"/>
                            <xsl:variable name="media_type">
                                <xsl:choose>
                                    <xsl:when test="starts-with($fmt,'all ebooks')">
                                        <xsl:value-of select="'electronic'"/>
                                    </xsl:when>
                                    <!--
                                    <xsl:when test="starts-with($fmt,'hardcover') or starts-with($fmt,'paper')">
                                        <xsl:value-of select="'print'"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="$format_list[$pos]"/>
                                    </xsl:otherwise>
                                    -->
                                    <xsl:otherwise>
                                        <xsl:value-of select="'print'"/>
                            ...
                            <xsl:if test="$media_type='print' or $media_type='electronic'">
                    ...
                    <xsl:element name="doi_data" namespace="{$NAMESPACE_URL}">
                        ...
                        <xsl:variable name="resourceValue">
                            <xsl:choose>
                                ...
                                -->
                                <xsl:when test="$pbisac_active='true' and ./ISBN1">
                                    <xsl:value-of select="concat($url_prefix, ./ISBN1)"/>
                                </xsl:when>
                                <xsl:when test="$sbisac_active='true' and ./ISBN2">
                                    <xsl:value-of select="concat($url_prefix, ./ISBN2)"/>
                                </xsl:when>
                                <xsl:otherwise>
