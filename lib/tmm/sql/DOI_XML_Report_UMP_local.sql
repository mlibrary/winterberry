...
CASE WHEN dbo.rpt_get_misc_value_check(whti.bookkey, 243/*Full Text on Fulcrum*/, 'Y') = 'Y' and (dbo.rpt_get_misc_value(ebook.bookkey,286/*Fulcrum Status*/,'L') = 'Published' or dbo.rpt_get_misc_value(currentOA.bookkey,286/*Fulcrum Status*/,'L') = 'Published') THEN dbo.rpt_get_misc_value(whti.bookkey, 250/*Fulcrum URL*/, 'long')
	 ...
	 END
'resource',
whti.primaryeditionworkkey 'workkey',doi.textvalue 'doi', currentOA.textvalue 'OAURL', case when whtc.groupentry2 = 'distributed' then whtc.groupentry3 end 'distClient'
...
--and dbo.rpt_get_misc_value(whti.bookkey,243/*Full Text on Fulcrum*/,'Y')='Yes'
and whtc.groupentry2 not in ('Michigan Publishing Services', 'MPS Distributed', 'MPS Hosted Clients')
and ptitle.ean13 is not null
and pubyear is not null
--and (ptitle.publishtowebind = 1 or subtitle.publishtowebind = 1 or ebook.publishtowebind = 1)
--and titleandtitleprefix = 'ESC'
--and dbo.rpt_get_misc_value(whti.bookkey,244,'long') like '2020'
--and whtc.groupentry3 in ('U OF M CENTER FOR JAPANESE STUDIES','U OF M CENTER FOR CHINESE STUDIES','U OF M CENTER FOR SOUTH EAST ASIAN STUDIES')
--and currentoa.PubDate = '2020-08-06'
--and ptitle.PubDate >= '2020-01-01'
