class ImgProcessor < FragmentProcessor
  ...
  def process(args = {})
    ...
    fragments.each do |fragment|
      ...
      nodes = fragment.node.xpath(".//*[local-name()='img']")
      if nodes.count == 0
        # Empty figure element. Not expected, skip.
        next
      end
      img_node = nodes.first

      nodes = fragment.node.xpath(".//*[@class='figcaption']")
      caption = nodes.count > 0 ? nodes.first.text : ""
      nodes = fragment.node.xpath(".//*[local-name()='p']")
      if nodes.count > 0
        caption += caption.empty? ? nodes.first.text : " " + nodes.first.text
      end
...
=begin
      ...
      nodes = fragment.node.xpath(".//*[local-name()='img' or local-name()='p' or @class='figcaption']")
      if nodes.count == 0
        # Empty figure element. Not expected, skip.
        next
      end
