module UMPTG::XHTML::Pipeline::Filter
  ...
  class MigrationFilter < UMPTG::XML::Pipeline::Filter
    ...
    def process_heading(reference_node, args)
      ...
      ctype_list = reference_node.xpath(".//*[local-name()='meta' and @content='text/html; charset=UTF-8']")
      unless ctype_list.empty?
        ctype_list.each do |n|
          actions << UMPTG::XML::Pipeline::Actions::SetAttributeValueAction.new(
                    ...
                    warning_message: "#{name}, invalid meta[@content=\"#{n['content']}\""
                  )
        end
      end
